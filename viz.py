...
def visualize(model, device, test_loader, criterion):
    ...
    with torch.no_grad():
        ...
        for vids in output:
            ...
            for ind,frame in enumerate(vids):

                # print(frame.shape)
                # name = "frame_"+str(ind).zfill(3)+".jpg"
                # cv2.imwrite(name,frame)

                plt.imshow(frame.cpu())
                plt.show()
...
def main(FLAGS):
    ...
    model.load_state_dict(torch.load("D:\\UCF\\Coursework\\CAP5415\\course project\\vae\\checkpoint_19.pth"))
    ...
    for epoch in range(1, FLAGS.num_epochs+1):
        ...
        test_loss = visualize(model, device, train_dataloader, criterion)
